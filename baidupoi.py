...
class Spider:
    ...
    def Work(self, maxThreads, baiduTiles, zoom):
        ...
        if (True):
            ...
            self.conn.commit()
            ShowInfo('{0} 共获得UID {1} 个'.format(tid, len(self.uidList)))
...
if __name__ == '__main__':
    ...
    maxThreads = 50                         # 线程的数量
    outpath = './output'                    # 输出路径
    dbfile = '%s/%s.db' % (outpath, tid)    # 数据库

    print '{0} -> TaskID:{1} [{2}, {3}, {4}, {5}]/{6} ...'.format(time.strftime(r'%m/%d %H:%M:%S'), tid, minX, maxX, minY, maxY, zoom)
    ...
    # 写任务日志
    tasklogfile = open(TASK_LOG_FILE, 'a')
    tasklogfile.write('%s %s\n' % (tid, success))
    tasklogfile.close()

    # 如果该任务获取的所有数据为空删除该任务对应的数据库文件
    if (dbisempty==True): os.remove(dbfile)
